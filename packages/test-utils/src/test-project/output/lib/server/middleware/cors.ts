...
function resolveRequestOrigin(
  ...
  const resolvedOrigin = resolveOrigin(options.origin, request.origin, options.credentials);

  return options.credentials && resolvedOrigin === "*" ? undefined : resolvedOrigin;
}
...
function shouldVaryDeniedCorsResponse(
  ...
  return (
    request.hasOrigin || isOriginDependentWithoutRequestOrigin(options.origin, options.credentials)
  );
}
...
function isPreflightCorsRequest(request: CorsRequest): boolean {
  return (
    ...
    readSingletonHeader(request.header, "access-control-request-method") !== undefined
  );
}
...
function buildPreflightCorsHeaders(
  ...
  if (options.allowHeaders !== undefined) {
    if (options.allowHeaders.length > 0) {
      corsHeaders["access-control-allow-headers"] = options.allowHeaders.join(", ");
    }
  } else {
    const requestedHeaders = readSingletonHeader(request.header, "access-control-request-headers");
...
function removePolicyControlledCorsHeaders(
  ...
  for (const [key, value] of Object.entries(responseHeaders ?? {})) {
    if (value === undefined || POLICY_CONTROLLED_CORS_HEADERS.has(key.toLowerCase())) {
      continue;
    }
