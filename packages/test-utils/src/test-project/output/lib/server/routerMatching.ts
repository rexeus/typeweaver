...
function writeCaptured(captured: Record<string, string>, params: Record<string, string>): string[] {
  const names = Object.keys(captured);
  for (const name of names) params[name] = captured[name] ?? "";
  return names;
}

function clearCaptured(names: readonly string[], params: Record<string, string>): void {
