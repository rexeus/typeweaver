...
export function enforceContentLengthLimit(req: IncomingMessage, maxBodySize: number): void {
...
export async function drainUnvalidatedRequestBody(
  ...
  const drainResult = await drainRequestStream(req, bodyLimitPolicy.maxBodySize, {
