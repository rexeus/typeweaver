...
export async function resolveAppRequest(options: {
  ...
  if (match) {
    ...
    try {
      ...
      return validateResponse(match.route, normalizeHttpResponse(response), routeCtx);
    } catch (error) {
...
function withPathParams(ctx: ServerContext, params: Record<string, string>): ServerContext {
