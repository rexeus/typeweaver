...
import { isHttpMethod } from "@rexeus/typeweaver-core";
import type {
  IHttpBody,
  IHttpHeader,
  IHttpResponse,
 ...
 */
export class FetchApiAdapter extends HttpAdapter<Request, Response, Record<string, string>> {
  /**
  ...
  public async toRequest(
    ...
    // Hono only dispatches requests to routes registered for an `HttpMethod`,
    // so this rejects only direct adapter calls with another method token.
  ...
  private async parseRequestBody(request: Request): Promise<IHttpBody> {
    ...
    if (contentType?.includes("application/x-www-form-urlencoded")) {
      ...
      const formObject = createNullPrototypeRecord<string | string[]>();
  ...
  private buildResponseBody(body: unknown): string | ArrayBuffer | Blob | null {
    ...
    try {
      ...
      throw new HonoResponseSerializationError("Failed to serialize response body to JSON", {
        cause: error,
      });
    }
  }
