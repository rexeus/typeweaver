...
const addValue = (values: Map<string, string[]>, option: string, value: string): void => {
...
const onlyValue = (parsed: ParsedArguments, option: string): string | undefined => {
...
const validateKnownOptions = (command: GeneratedCommand, parsed: ParsedArguments): void => {
...
const inputValues = (
  ...
  if (values.length !== 1) {
    throw new CommandUsageError(`Option '--${input.flag}' may be provided once.`);
  }
  return values[0];
};
...
const readBody = async (
  ...
  const selected = [inline !== undefined, filePath !== undefined, stdinRequested].filter(
    Boolean,
  ).length;
  if (selected > 1) {
    throw new CommandUsageError("Use only one of '--body', '--body-file', or '--body-stdin'.");
  }
  if (!command.hasBody) {
    if (selected > 0) {
      throw new CommandUsageError(`Command '${command.name}' does not accept a request body.`);
    }
    return undefined;
  }
...
const selectedSecuritySchemes = (
  ...
  if (requirement === undefined) {
    const alternatives = command.security.requirements
      .map((names) =>
        names
          .map((name) => command.security.schemes.find((item) => item.name === name))
          .filter((scheme) => scheme !== undefined)
          .map((scheme) => `--${scheme.flag}`)
          .join(" + "),
      )
      .join(" or ");
    throw new CommandUsageError(`Missing authentication. Provide ${alternatives}.`);
  }
...
const applySecurityScheme = (
  ...
  if (scheme.location === "header") {
    ...
    headers.Cookie = headers.Cookie === undefined ? entry : `${headers.Cookie}; ${entry}`;
  }
};

const resolveSecurity = (command: GeneratedCommand, parsed: ParsedArguments): SecurityValues => {
...
const helpText = (program: GeneratedCommandProgram, command?: GeneratedCommand): string => {
  ...
  return [
    ...
    ...(command.hasBody ? ["  --body <value>", "  --body-file <path>", "  --body-stdin"] : []),
    "  --human",
    "  --help",
    "",
  ].join("\n");
...
const writeSuccess = (
  ...
  if (human) {
    ...
    io.writeStdout(`${response.statusCode} ${response.type} (${command.operationId})${body}\n`);
    return;
  }
  io.writeStdout(`${JSON.stringify({ ok: true, operationId: command.operationId, response })}\n`);
};
...
const isNetworkError = (error: unknown): error is Error & { readonly code: string } =>
