...
import { type UploadFileResponse, type IUploadFileSuccessResponse } from "./UploadFileResponse";
...
export class UploadFileResponseValidator extends ResponseValidator {
  public safeValidate(response: IHttpResponse): SafeResponseValidationResult<UploadFileResponse> {
  ...
  public validate(response: IHttpResponse): UploadFileResponse {
  ...
  ): SafeResponseValidationResult<UploadFileResponse> {
    const error = new ResponseValidationError(response.statusCode);

    if (response.statusCode === 201) {
      const validateUploadFileSuccessResponseResult = this.validateUploadFileSuccessResponse(
        response,
        error,
      );
      if (validateUploadFileSuccessResponseResult.isValid) {
        return validateUploadFileSuccessResponseResult;
      }
    }
  ...
  private validateUploadFileSuccessResponse(
    response: IHttpResponse,
    error: ResponseValidationError,
  ): SafeResponseValidationResult<IUploadFileSuccessResponse> {
    const result = this.validateResponseType<IUploadFileSuccessResponse>(
      "UploadFileSuccess",
    ...
    return {
      isValid: true,
      data: {
        _tag: "UploadFileSuccess" as const,
        statusCode: result.data.statusCode,
        header: result.data.header,
        body: result.data.body,
      } as IUploadFileSuccessResponse,
    };
  }
