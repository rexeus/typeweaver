...
import { type OptionsTodoResponse, type IOptionsTodoSuccessResponse } from "./OptionsTodoResponse";
...
export class OptionsTodoResponseValidator extends ResponseValidator {
  public safeValidate(response: IHttpResponse): SafeResponseValidationResult<OptionsTodoResponse> {
  ...
  private validateAgainstDefinedResponses(
    ...
    if (response.statusCode === 200) {
      const validateOptionsTodoSuccessResponseResult = this.validateOptionsTodoSuccessResponse(
        response,
        error,
      );
      if (validateOptionsTodoSuccessResponseResult.isValid) {
        return validateOptionsTodoSuccessResponseResult;
      }
    }
  ...
  private validateOptionsTodoSuccessResponse(
  ...
  ): SafeResponseValidationResult<IOptionsTodoSuccessResponse> {
    const result = this.validateResponseType<IOptionsTodoSuccessResponse>(
      "OptionsTodoSuccess",
    ...
    return {
      isValid: true,
      data: {
        type: "OptionsTodoSuccess" as const,
        statusCode: result.data.statusCode,
        header: result.data.header,
        body: result.data.body,
      } as IOptionsTodoSuccessResponse,
    };
  }
