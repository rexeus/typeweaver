...
import {
  ...
  type IOptionsTodoSuccessResponse,
...
export class OptionsTodoResponseValidator extends ResponseValidator {
  public safeValidate(
    response: IHttpResponse,
  ): SafeResponseValidationResult<OptionsTodoResponse> {
    ...
    let data: OptionsTodoResponse;
    switch (response.statusCode) {
      case 200: {
        if (validationResult.validResponseName === "OptionsTodoSuccess") {
          data = new OptionsTodoSuccessResponse(
            validationResult.validatedResponse as unknown as IOptionsTodoSuccessResponse,
          );
          break;
        }
  ...
  public validate(response: IHttpResponse): OptionsTodoResponse {
  ...
  private validateAgainstDefinedResponses(
    ...
    if (response.statusCode === 200) {
      const isOptionsTodoSuccessResponse =
        this.validateOptionsTodoSuccessResponse(
          response,
          validatedResponse,
          error,
        );
      if (isOptionsTodoSuccessResponse) {
        return { validResponseName: "OptionsTodoSuccess", validatedResponse };
      }
    }
  ...
  private validateOptionsTodoSuccessResponse(
    response: IHttpResponse,
    ...
    assert(
      ...
      "'OptionsTodoSuccessResponseHeader' has to be defined in the definition",
    );
