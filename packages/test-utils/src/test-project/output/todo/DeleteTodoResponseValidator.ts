...
import { type DeleteTodoResponse, type IDeleteTodoSuccessResponse } from "./DeleteTodoResponse";
...
export class DeleteTodoResponseValidator extends ResponseValidator {
  public safeValidate(response: IHttpResponse): SafeResponseValidationResult<DeleteTodoResponse> {
    const result = this.validateAgainstDefinedResponses(response);

    if (!result.isValid && !result.error.hasResponseIssues()) {
      result.error.addStatusCodeIssue([204, 400, 401, 403, 404, 415, 429, 500]);
    }
  ...
  private validateAgainstDefinedResponses(
    ...
    if (response.statusCode === 204) {
      const validateDeleteTodoSuccessResponseResult = this.validateDeleteTodoSuccessResponse(
        response,
        error,
      );
      if (validateDeleteTodoSuccessResponseResult.isValid) {
        return validateDeleteTodoSuccessResponseResult;
      }
    }
  ...
  private validateDeleteTodoSuccessResponse(
  ...
  ): SafeResponseValidationResult<IDeleteTodoSuccessResponse> {
    const result = this.validateResponseType<IDeleteTodoSuccessResponse>(
      "DeleteTodoSuccess",
    ...
    return {
      isValid: true,
      data: {
        _tag: "DeleteTodoSuccess" as const,
        statusCode: result.data.statusCode,
        header: result.data.header,
        body: result.data.body,
      } as IDeleteTodoSuccessResponse,
    };
  }
