...
export class HeadTodoResponseValidator extends ResponseValidator {
  public safeValidate(
    ...
    let data: HeadTodoResponse;
    switch (response.statusCode) {
      case 200: {
        if (validationResult.validResponseName === "HeadTodoSuccess") {
          data = new HeadTodoSuccessResponse(
            validationResult.validatedResponse as unknown as IHeadTodoSuccessResponse,
          );
          break;
        }
  ...
  private validateAgainstDefinedResponses(
    ...
    if (response.statusCode === 200) {
      const isHeadTodoSuccessResponse = this.validateHeadTodoSuccessResponse(
        response,
        validatedResponse,
        error,
      );
      if (isHeadTodoSuccessResponse) {
        return { validResponseName: "HeadTodoSuccess", validatedResponse };
      }
    }
