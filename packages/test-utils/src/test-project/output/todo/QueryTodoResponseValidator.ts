...
export class QueryTodoResponseValidator extends ResponseValidator {
  public safeValidate(
    ...
    let data: QueryTodoResponse;
    switch (response.statusCode) {
      case 200: {
        if (validationResult.validResponseName === "QueryTodoSuccess") {
          data = new QueryTodoSuccessResponse(
            validationResult.validatedResponse as unknown as IQueryTodoSuccessResponse,
          );
          break;
        }
  ...
  private validateAgainstDefinedResponses(
    ...
    if (response.statusCode === 200) {
      const isQueryTodoSuccessResponse = this.validateQueryTodoSuccessResponse(
        response,
        validatedResponse,
        error,
      );
      if (isQueryTodoSuccessResponse) {
        return { validResponseName: "QueryTodoSuccess", validatedResponse };
      }
    }
  ...
  private validateQueryTodoSuccessResponse(
    ...
    assert(
      ...
      "'QueryTodoSuccessResponseBody' has to be defined in the definition",
    );
    ...
    assert(
      ...
      "'QueryTodoSuccessResponseHeader' has to be defined in the definition",
    );
