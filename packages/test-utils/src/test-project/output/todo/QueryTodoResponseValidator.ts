...
import { type QueryTodoResponse, type IQueryTodoSuccessResponse } from "./QueryTodoResponse";
...
export class QueryTodoResponseValidator extends ResponseValidator {
  public safeValidate(response: IHttpResponse): SafeResponseValidationResult<QueryTodoResponse> {
  ...
  private validateAgainstDefinedResponses(
    ...
    if (response.statusCode === 200) {
      const validateQueryTodoSuccessResponseResult = this.validateQueryTodoSuccessResponse(
        response,
        error,
      );
  ...
  private validateQueryTodoSuccessResponse(
  ...
  ): SafeResponseValidationResult<IQueryTodoSuccessResponse> {
    const result = this.validateResponseType<IQueryTodoSuccessResponse>(
      "QueryTodoSuccess",
    ...
    return {
      isValid: true,
      data: {
        type: "QueryTodoSuccess" as const,
        statusCode: result.data.statusCode,
        header: result.data.header,
        body: result.data.body,
      } as IQueryTodoSuccessResponse,
    };
  }
