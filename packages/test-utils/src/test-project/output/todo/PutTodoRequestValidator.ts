...
export class PutTodoRequestValidator extends RequestValidator<IPutTodoRequest> {
  public safeValidate(request: IRawHttpRequest): SafeRequestValidationResult<IPutTodoRequest> {
    const error = new RequestValidationError();

    const bodySchema = this.requireRequestSchema(definition.request.body, "body");
    const bodyResult = this.safeParseAs<IPutTodoRequestBody>(bodySchema, request.body);
    ...
    const headerResult = this.safeParseAs<IPutTodoRequestHeader>(headerSchema, coercedHeader);
    ...
    const paramResult = this.safeParseAs<IPutTodoRequestParam>(paramSchema, request.param);
  ...
  public validate(request: IRawHttpRequest): IPutTodoRequest {
