...
import {
  ...
  type ICreateTodoSuccessResponseBody,
  type ICreateTodoSuccessResponseHeader,
...
export class CreateTodoResponseValidator extends ResponseValidator {
  ...
  private validateCreateTodoSuccessResponse(
    ...
    const validateBodyResult = definition.responses[0].body.safeParse(
      response.body,
    ) as unknown as ZodSafeParseResult<ICreateTodoSuccessResponseBody>;

    if (!validateBodyResult.success) {
      error.addBodyIssues("CreateTodoSuccess", validateBodyResult.error.issues);
      isValid = false;
    } else {
    ...
    const validateHeaderResult = definition.responses[0].header.safeParse(
      coercedHeader,
    ) as unknown as ZodSafeParseResult<ICreateTodoSuccessResponseHeader>;
    ...
    return {
      ...
      data: new CreateTodoSuccessResponse(
        validatedResponse as ICreateTodoSuccessResponse,
      ),
    };
  }
