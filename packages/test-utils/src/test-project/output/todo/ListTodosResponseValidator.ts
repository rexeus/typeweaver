...
import {
  ...
  type IListTodosSuccessResponseBody,
  type IListTodosSuccessResponseHeader,
...
export class ListTodosResponseValidator extends ResponseValidator {
  ...
  private validateAgainstDefinedResponses(
    ...
    if (response.statusCode === 200) {
      const validateListTodosSuccessResponseResult =
        this.validateListTodosSuccessResponse(response, error);
      if (validateListTodosSuccessResponseResult.isValid) {
        return validateListTodosSuccessResponseResult;
      }
    }
  ...
  private validateListTodosSuccessResponse(
  ...
  ): SafeResponseValidationResult<ListTodosSuccessResponse> {
    let isValid = true;
    ...
    const validateBodyResult = definition.responses[0].body.safeParse(
      response.body,
    ) as unknown as ZodSafeParseResult<IListTodosSuccessResponseBody>;

    if (!validateBodyResult.success) {
      error.addBodyIssues("ListTodosSuccess", validateBodyResult.error.issues);
      isValid = false;
    } else {
    ...
    const validateHeaderResult = definition.responses[0].header.safeParse(
      coercedHeader,
    ) as unknown as ZodSafeParseResult<IListTodosSuccessResponseHeader>;
    ...
    return {
      ...
      data: new ListTodosSuccessResponse(
        validatedResponse as IListTodosSuccessResponse,
      ),
    };
  }
