...
import {
  ...
  type IQuerySubTodoSuccessResponse,
...
export class QuerySubTodoResponseValidator extends ResponseValidator {
  public safeValidate(
    response: IHttpResponse,
  ): SafeResponseValidationResult<QuerySubTodoResponse> {
    ...
    let data: QuerySubTodoResponse;
    switch (response.statusCode) {
      case 200: {
        if (validationResult.validResponseName === "QuerySubTodoSuccess") {
          data = new QuerySubTodoSuccessResponse(
            validationResult.validatedResponse as unknown as IQuerySubTodoSuccessResponse,
          );
          break;
        }
  ...
  public validate(response: IHttpResponse): QuerySubTodoResponse {
  ...
  private validateAgainstDefinedResponses(
    ...
    if (response.statusCode === 200) {
      const isQuerySubTodoSuccessResponse =
        this.validateQuerySubTodoSuccessResponse(
          response,
          validatedResponse,
          error,
        );
      if (isQuerySubTodoSuccessResponse) {
        return { validResponseName: "QuerySubTodoSuccess", validatedResponse };
      }
    }
  ...
  private validateQuerySubTodoSuccessResponse(
    response: IHttpResponse,
    ...
    assert(
      ...
      "'QuerySubTodoSuccessResponseBody' has to be defined in the definition",
    );
    ...
    assert(
      ...
      "'QuerySubTodoSuccessResponseHeader' has to be defined in the definition",
    );
