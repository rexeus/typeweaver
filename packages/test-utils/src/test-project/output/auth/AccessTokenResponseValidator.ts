...
import { type AccessTokenResponse, type IAccessTokenSuccessResponse } from "./AccessTokenResponse";
...
export class AccessTokenResponseValidator extends ResponseValidator {
  public safeValidate(response: IHttpResponse): SafeResponseValidationResult<AccessTokenResponse> {
  ...
  private validateAgainstDefinedResponses(
    ...
    if (response.statusCode === 200) {
      const validateAccessTokenSuccessResponseResult = this.validateAccessTokenSuccessResponse(
        response,
        error,
      );
  ...
  private validateAccessTokenSuccessResponse(
  ...
  ): SafeResponseValidationResult<IAccessTokenSuccessResponse> {
    const result = this.validateResponseType<IAccessTokenSuccessResponse>(
      "AccessTokenSuccess",
    ...
    return {
      isValid: true,
      data: {
        type: "AccessTokenSuccess" as const,
        statusCode: result.data.statusCode,
        header: result.data.header,
        body: result.data.body,
      } as IAccessTokenSuccessResponse,
    };
  }
