...
import {
  ...
  type IAccessTokenSuccessResponseBody,
  type IAccessTokenSuccessResponseHeader,
...
export class AccessTokenResponseValidator extends ResponseValidator {
  ...
  private validateAgainstDefinedResponses(
    ...
    if (response.statusCode === 200) {
      const validateAccessTokenSuccessResponseResult =
        this.validateAccessTokenSuccessResponse(response, error);
      if (validateAccessTokenSuccessResponseResult.isValid) {
        return validateAccessTokenSuccessResponseResult;
      }
    }
  ...
  private validateAccessTokenSuccessResponse(
  ...
  ): SafeResponseValidationResult<AccessTokenSuccessResponse> {
    let isValid = true;
    ...
    const validateBodyResult = definition.responses[0].body.safeParse(
      response.body,
    ) as unknown as ZodSafeParseResult<IAccessTokenSuccessResponseBody>;
    ...
    const validateHeaderResult = definition.responses[0].header.safeParse(
      coercedHeader,
    ) as unknown as ZodSafeParseResult<IAccessTokenSuccessResponseHeader>;
    ...
    return {
      ...
      data: new AccessTokenSuccessResponse(
        validatedResponse as IAccessTokenSuccessResponse,
      ),
    };
  }
