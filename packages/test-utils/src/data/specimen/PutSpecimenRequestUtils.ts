...
import type { IPutSpecimenRequest } from "../..";
...
export const createPutSpecimenRequestBody =
  createDataFactory<IPutSpecimenRequestBody>(() => {
    ...
    return {
      ...
      literalField: "test",
      enumField: faker.helpers.arrayElement([
        "ACTIVE",
        "INACTIVE",
        "PENDING",
      ] as const),
      templateLiteralField: `This is the current status ${faker.lorem.word()} is ${faker.helpers.arrayElement(["active", "inactive"] as const)}`,
      ...
      nullableField: faker.helpers.arrayElement([faker.lorem.word(), null]),
...
export const createPutSpecimenRequestQuery =
  createDataFactory<IPutSpecimenRequestQuery>(() => ({
