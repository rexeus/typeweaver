    ...
    this.config = {
      ...
      errorHandlers: {
        requestValidation: resolveHonoErrorHandler(
          handleRequestValidationErrors,
          (error, context) =>
            this.defaultHandlers.requestValidation(error, context)
        ),
        bodyParse: resolveHonoErrorHandler(
          handleBodyParseErrors,
          (error, context) => this.defaultHandlers.bodyParse(error, context)
        ),
        ...
        httpResponse: resolveHonoErrorHandler(
          handleHttpResponseErrors,
          (error, context) => this.defaultHandlers.httpResponse(error, context)
        ),
        unknown: resolveHonoErrorHandler(
          handleUnknownErrors,
          (error, context) => this.defaultHandlers.unknown(error, context)
        ),
      },
    };
  }
      ...
      if (error instanceof HonoBodyParseError) {
        ...
        if (bodyParseHandler) {
          ...
          return this.adapter.toResponse(
            response ?? defaultBodyParseHandler(error)
          );
        }
      }
