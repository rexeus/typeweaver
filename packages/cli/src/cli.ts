...
export const createCli = (): Command => {
  ...
  program
    ...
    .addOption(new Option("--verbose", "show debug diagnostics").conflicts("quiet"))
    .addOption(new Option("--quiet", "suppress non-error output").conflicts("verbose"))
    .option("--no-color", "disable ANSI colors in CLI output");

  program
    .command("doctor")
    .description("Run read-only setup diagnostics for the current Typeweaver project")
    .option(
      "-c, --config <configFile>",
      "path to a .js, .mjs, or .cjs configuration file"
    )
    .option("--deep", "run deeper spec bundle/import checks in a temp directory")
    .action(async (_options, command) => {
      await handleDoctorCommand(command.optsWithGlobals() as DoctorCommandOptions);
    });
  ...
  program
    ...
    .option("-o, --output <outputDir>", "output directory for generated files")
    .option("-p, --plugins <plugins>", "comma-separated list of plugins to add to the generated config")
  ...
  program
    .command("migrate")
    .description("Show read-only migration guidance for older Typeweaver projects")
    .option("--from <version>", "override the detected Typeweaver version")
    .action(async (_options, command) => {
      await handleMigrateCommand(command.optsWithGlobals() as MigrateCommandOptions);
    });
