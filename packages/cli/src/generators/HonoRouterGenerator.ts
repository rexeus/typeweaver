...
export class HonoRouterGenerator {
  public static generate(entityResources: EntityResources): void {
    const templateFile = path.join(Generator.templateDir, "HonoRouter.ejs");
    ...
    for (const [entityName, operationResources] of Object.entries(
      ...
      this.writeHonoRoutes(entityName, template, operationResources);
    }
  }

  private static writeHonoRoutes(
    entityName: string,
    template: string,
    ...
    const outputFile = path.join(
      outputDir,
      `${pascalCaseEntityName}HonoRouter.ts`
    );
