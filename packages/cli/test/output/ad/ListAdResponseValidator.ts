...
export class ListAdResponseValidator extends ResponseValidator {
  public safeValidate(
    response: IHttpResponse,
  ): SafeResponseValidationResult<ListAdResponse> {
    ...
    let data: ListAdResponse;
    switch (response.statusCode) {
      case 200: {
        if (validationResult.validResponseName === "ListAdSuccess") {
          data = new ListAdSuccessResponse(
            validationResult.validatedResponse as unknown as IListAdSuccessResponse,
          );
          break;
        }
  ...
  public validate(response: IHttpResponse): ListAdResponse {
  ...
  private validateAgainstDefinedResponses(
    ...
    if (response.statusCode === 200) {
      const isListAdSuccessResponse = this.validateListAdSuccessResponse(
        response,
        validatedResponse,
        error,
      );
      if (isListAdSuccessResponse) {
        return { validResponseName: "ListAdSuccess", validatedResponse };
      }
    }
  ...
  private validateListAdSuccessResponse(
    response: IHttpResponse,
    ...
    assert(
      ...
      "'ListAdSuccessResponseBody' has to be defined in the definition",
    );
    ...
    assert(
      ...
      "'ListAdSuccessResponseHeader' has to be defined in the definition",
    );
