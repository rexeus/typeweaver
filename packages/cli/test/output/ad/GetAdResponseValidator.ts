...
import {
  type GetAdResponse,
  type IGetAdSuccessResponse,
  GetAdSuccessResponse,
} from "./GetAdResponse";

import {
  type IAdNotFoundErrorResponse,
...
export class GetAdResponseValidator extends ResponseValidator {
  public safeValidate(
    response: IHttpResponse,
  ): SafeResponseValidationResult<GetAdResponse> {
    ...
    let data: GetAdResponse;
    switch (response.statusCode) {
      case 200: {
        if (validationResult.validResponseName === "GetAdSuccess") {
          data = new GetAdSuccessResponse(
            validationResult.validatedResponse as unknown as IGetAdSuccessResponse,
          );
          break;
        }
      ...
      case 404: {
        if (validationResult.validResponseName === "AdNotFoundError") {
          data = new AdNotFoundErrorResponse(
            validationResult.validatedResponse as unknown as IAdNotFoundErrorResponse,
          );
          break;
        }
  ...
  public validate(response: IHttpResponse): GetAdResponse {
  ...
  private validateAgainstDefinedResponses(
    ...
    if (response.statusCode === 200) {
      const isGetAdSuccessResponse = this.validateGetAdSuccessResponse(
        response,
        validatedResponse,
        error,
      );
      if (isGetAdSuccessResponse) {
        return { validResponseName: "GetAdSuccess", validatedResponse };
      }
    }

    if (response.statusCode === 404) {
      const isAdNotFoundErrorResponse = this.validateAdNotFoundErrorResponse(
        response,
        validatedResponse,
        error,
      );
      if (isAdNotFoundErrorResponse) {
        return { validResponseName: "AdNotFoundError", validatedResponse };
      }
    }
  ...
  private validateGetAdSuccessResponse(
    response: IHttpResponse,
    ...
    assert(
      ...
      "'GetAdSuccessResponseBody' has to be defined in the definition",
    );
    ...
    assert(
      ...
      "'GetAdSuccessResponseHeader' has to be defined in the definition",
    );
  ...
  private validateAdNotFoundErrorResponse(
    response: IHttpResponse,
    validatedResponse: IHttpResponse,
    error: ResponseValidationError,
  ): boolean {
    ...
    assert(
      ...
      "'AdNotFoundErrorResponseBody' has to be defined in the definition",
    );
    ...
    assert(
      ...
      "'AdNotFoundErrorResponseHeader' has to be defined in the definition",
    );
