...
export class LogoutResponseValidator extends ResponseValidator {
  public safeValidate(
    response: IHttpResponse,
  ): SafeResponseValidationResult<LogoutResponse> {
    ...
    switch (response.statusCode) {
      case 204: {
        if (validationResult.validResponseName === "LogoutSuccess") {
          data = new LogoutSuccessResponse(
            validationResult.validatedResponse as unknown as ILogoutSuccessResponse,
          );
          break;
        }
  ...
  public validate(response: IHttpResponse): LogoutResponse {
  ...
  private validateAgainstDefinedResponses(
    ...
    if (response.statusCode === 204) {
      const isLogoutSuccessResponse = this.validateLogoutSuccessResponse(
        response,
        validatedResponse,
        error,
      );
      if (isLogoutSuccessResponse) {
        return { validResponseName: "LogoutSuccess", validatedResponse };
      }
    }
  ...
  private validateLogoutSuccessResponse(
    response: IHttpResponse,
    ...
    assert(
      ...
      "'LogoutSuccessResponseHeader' has to be defined in the definition",
    );
