...
import {
  ...
  type ICreateProjectSuccessResponse,
...
export class CreateProjectResponseValidator extends ResponseValidator {
  public safeValidate(
    response: IHttpResponse,
  ): SafeResponseValidationResult<CreateProjectResponse> {
    ...
    switch (response.statusCode) {
      case 200: {
        if (validationResult.validResponseName === "CreateProjectSuccess") {
          data = new CreateProjectSuccessResponse(
            validationResult.validatedResponse as unknown as ICreateProjectSuccessResponse,
          );
          break;
        }
  ...
  public validate(response: IHttpResponse): CreateProjectResponse {
  ...
  private validateAgainstDefinedResponses(
    ...
    if (response.statusCode === 200) {
      const isCreateProjectSuccessResponse =
        this.validateCreateProjectSuccessResponse(
          response,
          validatedResponse,
          error,
        );
      if (isCreateProjectSuccessResponse) {
        return { validResponseName: "CreateProjectSuccess", validatedResponse };
      }
    }
  ...
  private validateCreateProjectSuccessResponse(
    response: IHttpResponse,
    ...
    assert(
      ...
      "'CreateProjectSuccessResponseBody' has to be defined in the definition",
    );
    ...
    assert(
      ...
      "'CreateProjectSuccessResponseHeader' has to be defined in the definition",
    );
