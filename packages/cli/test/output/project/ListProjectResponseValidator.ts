...
import {
  ...
  type IListProjectSuccessResponse,
...
export class ListProjectResponseValidator extends ResponseValidator {
  public safeValidate(
    response: IHttpResponse,
  ): SafeResponseValidationResult<ListProjectResponse> {
    ...
    let data: ListProjectResponse;
    switch (response.statusCode) {
      case 200: {
        if (validationResult.validResponseName === "ListProjectSuccess") {
          data = new ListProjectSuccessResponse(
            validationResult.validatedResponse as unknown as IListProjectSuccessResponse,
          );
          break;
        }
  ...
  public validate(response: IHttpResponse): ListProjectResponse {
  ...
  private validateAgainstDefinedResponses(
    ...
    if (response.statusCode === 200) {
      const isListProjectSuccessResponse =
        this.validateListProjectSuccessResponse(
          response,
          validatedResponse,
          error,
        );
      if (isListProjectSuccessResponse) {
        return { validResponseName: "ListProjectSuccess", validatedResponse };
      }
    }
  ...
  private validateListProjectSuccessResponse(
    response: IHttpResponse,
    ...
    assert(
      ...
      "'ListProjectSuccessResponseBody' has to be defined in the definition",
    );
    ...
    assert(
      ...
      "'ListProjectSuccessResponseHeader' has to be defined in the definition",
    );
