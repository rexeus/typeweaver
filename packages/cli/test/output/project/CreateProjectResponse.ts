...
export type ICreateProjectSuccessResponseHeader = {
  "Content-Type": "application/json";
};

export type ICreateProjectSuccessResponseBody = {
  id: string;
  accountId: string;
  title: string;
  status:
...
export type ICreateProjectSuccessResponse = {
  statusCode: HttpStatusCode.OK;
  header: ICreateProjectSuccessResponseHeader;
  body: ICreateProjectSuccessResponseBody;
};

export class CreateProjectSuccessResponse
  extends HttpResponse<
    ICreateProjectSuccessResponseHeader,
    ICreateProjectSuccessResponseBody
  >
  implements ICreateProjectSuccessResponse
{
  public override readonly statusCode: HttpStatusCode.OK;

  public constructor(response: ICreateProjectSuccessResponse) {
    super(response.statusCode, response.header, response.body);

    if (response.statusCode !== HttpStatusCode.OK) {
      throw new Error(
        `Invalid status code: '${response.statusCode}' for CreateProjectSuccessResponse`,
      );
    }
...
export type ICreateProjectSuccessResponses = ICreateProjectSuccessResponse;

export type CreateProjectSuccessResponses = CreateProjectSuccessResponse;

export type ICreateProjectResponse =
  | ICreateProjectSuccessResponse
  | IForbiddenErrorResponse
  | IInternalServerErrorResponse
  | ITooManyRequestsErrorResponse
  | IUnauthorizedErrorResponse
