...
import {
  ...
  type IGetFunnelSuccessResponse,
...
export class GetFunnelResponseValidator extends ResponseValidator {
  public safeValidate(
    response: IHttpResponse,
  ): SafeResponseValidationResult<GetFunnelResponse> {
    ...
    let data: GetFunnelResponse;
    switch (response.statusCode) {
      case 200: {
        if (validationResult.validResponseName === "GetFunnelSuccess") {
          data = new GetFunnelSuccessResponse(
            validationResult.validatedResponse as unknown as IGetFunnelSuccessResponse,
          );
          break;
        }
  ...
  public validate(response: IHttpResponse): GetFunnelResponse {
  ...
  private validateAgainstDefinedResponses(
    ...
    if (response.statusCode === 200) {
      const isGetFunnelSuccessResponse = this.validateGetFunnelSuccessResponse(
        response,
        validatedResponse,
        error,
      );
      if (isGetFunnelSuccessResponse) {
        return { validResponseName: "GetFunnelSuccess", validatedResponse };
      }
    }
  ...
  private validateGetFunnelSuccessResponse(
    response: IHttpResponse,
    ...
    assert(
      ...
      "'GetFunnelSuccessResponseBody' has to be defined in the definition",
    );
    ...
    assert(
      ...
      "'GetFunnelSuccessResponseHeader' has to be defined in the definition",
    );
