...
import {
  ...
  type IGetPublicFunnelSuccessResponse,
  GetPublicFunnelSuccessResponse,
} from "./GetPublicFunnelResponse";

import {
  type IFunnelNotFoundErrorResponse,
...
export class GetPublicFunnelResponseValidator extends ResponseValidator {
  public safeValidate(
    response: IHttpResponse,
  ): SafeResponseValidationResult<GetPublicFunnelResponse> {
    ...
    let data: GetPublicFunnelResponse;
    switch (response.statusCode) {
      case 200: {
        if (validationResult.validResponseName === "GetPublicFunnelSuccess") {
          data = new GetPublicFunnelSuccessResponse(
            validationResult.validatedResponse as unknown as IGetPublicFunnelSuccessResponse,
          );
          break;
        }
      ...
      case 404: {
        if (validationResult.validResponseName === "FunnelNotFoundError") {
          data = new FunnelNotFoundErrorResponse(
            validationResult.validatedResponse as unknown as IFunnelNotFoundErrorResponse,
          );
          break;
        }
  ...
  public validate(response: IHttpResponse): GetPublicFunnelResponse {
  ...
  private validateAgainstDefinedResponses(
    ...
    if (response.statusCode === 200) {
      const isGetPublicFunnelSuccessResponse =
        this.validateGetPublicFunnelSuccessResponse(
          response,
          validatedResponse,
          error,
        );
      if (isGetPublicFunnelSuccessResponse) {
        return {
          validResponseName: "GetPublicFunnelSuccess",
          validatedResponse,
        };
      }
    }
    ...
    if (response.statusCode === 404) {
      const isFunnelNotFoundErrorResponse =
        this.validateFunnelNotFoundErrorResponse(
          response,
          validatedResponse,
          error,
        );
      if (isFunnelNotFoundErrorResponse) {
        return { validResponseName: "FunnelNotFoundError", validatedResponse };
      }
    }
  ...
  private validateGetPublicFunnelSuccessResponse(
    response: IHttpResponse,
    ...
    assert(
      ...
      "'GetPublicFunnelSuccessResponseBody' has to be defined in the definition",
    );
    ...
    assert(
      ...
      "'GetPublicFunnelSuccessResponseHeader' has to be defined in the definition",
    );
  ...
  private validateFunnelNotFoundErrorResponse(
    response: IHttpResponse,
    validatedResponse: IHttpResponse,
    error: ResponseValidationError,
  ): boolean {
    ...
    assert(
      ...
      "'FunnelNotFoundErrorResponseBody' has to be defined in the definition",
    );
    ...
    assert(
      ...
      "'FunnelNotFoundErrorResponseHeader' has to be defined in the definition",
    );
