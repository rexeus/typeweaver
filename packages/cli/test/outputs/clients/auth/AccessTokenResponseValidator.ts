...
import {
  ...
  type IAccessTokenSuccessResponse,
...
export class AccessTokenResponseValidator extends ResponseValidator {
  public safeValidate(
    response: IHttpResponse,
  ): SafeResponseValidationResult<AccessTokenResponse> {
    ...
    switch (response.statusCode) {
      case 200: {
        if (validationResult.validResponseName === "AccessTokenSuccess") {
          data = new AccessTokenSuccessResponse(
            validationResult.validatedResponse as unknown as IAccessTokenSuccessResponse,
          );
          break;
        }
  ...
  public validate(response: IHttpResponse): AccessTokenResponse {
  ...
  private validateAgainstDefinedResponses(
    ...
    if (response.statusCode === 200) {
      const isAccessTokenSuccessResponse =
        this.validateAccessTokenSuccessResponse(
          response,
          validatedResponse,
          error,
        );
      if (isAccessTokenSuccessResponse) {
        return { validResponseName: "AccessTokenSuccess", validatedResponse };
      }
    }
  ...
  private validateAccessTokenSuccessResponse(
    response: IHttpResponse,
    ...
    assert(
      ...
      "'AccessTokenSuccessResponseBody' has to be defined in the definition",
    );
    ...
    assert(
      ...
      "'AccessTokenSuccessResponseHeader' has to be defined in the definition",
    );
