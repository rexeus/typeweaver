...
const INTERNAL_SERVER_ERROR_BODY = createDefaultErrorBody(
  internalServerErrorDefaultError
);
...
export const defaultResponseValidationHandler: ResponseValidationErrorHandler =
  (): IHttpResponse =>
    createDefaultErrorResponse(internalServerErrorDefaultError);

export const defaultHttpResponseHandler: HttpResponseErrorHandler = err =>
  toHttpResponse(err);

export const defaultUnknownHandler: UnknownErrorHandler =
  (): IHttpResponse => ({
...
export async function handleAppError(options: {
  ...
  const unknownResponse = await handleUnknownError(
    error,
    config,
    ctx,
    safeOnError
...
async function handleRequestValidationError(
  ...
  return handler
    ? safelyExecuteErrorHandler(() => handler(error, ctx), safeOnError)
    : null;
}
