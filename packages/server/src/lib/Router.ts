...
export class Router {
  ...
  private static sameParameterNames(
    left: SegmentPattern,
  ...
  private static parseSegment(segment: string, path: string): SegmentPattern {
    ...
    const shape = JSON.stringify(
      tokens.map(token =>
        token.kind === "static" ? ["s", token.value] : ["p"]
      )
    );
  ...
  private static readPlaceholderName(
    segment: string,
    start: number
  ): string | undefined {
    let end = start;
  ...
  private static matchSegment(
    ...
    for (let index = 0; index < pattern.tokens.length; index += 1) {
      ...
      const matched = Router.matchParamToken(
