...
export class ExpressRouterGenerator {
  public static generate(context: GeneratorContext): void {
    ...
    const templateFile = path.join(moduleDir, "templates", "ExpressRouter.ejs");

    for (const [entityName, entityResource] of Object.entries(
      context.resources.entityResources
    )) {
      this.writeExpressRouter(
        entityName,
        templateFile,
  ...
  private static writeExpressRouter(
    entityName: string,
    templateFile: string,
    operationResources: readonly OperationResource[],
    ...
    const outputPath = path.join(
      outputDir,
      `${pascalCaseEntityName}Express.ts`
    );

    // Note: Unlike Hono, Express does NOT handle HEAD requests automatically
    // from GET routes, so we include all operations including HEAD
  ...
  private static compareRoutes(
    a: ReturnType<typeof ExpressRouterGenerator.createOperationData>,
    b: ReturnType<typeof ExpressRouterGenerator.createOperationData>
  ): number {
