...
`z.record(...)` remain supported. Array elements are inspected, so
...
`constructor` and `toString` remain supported in records and path parameters.
...
validator normalizes back to `[]` for array header schemas.
