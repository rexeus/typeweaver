...
| Normalized body warnings lose their resource and operation location when converted to validation issues               | Review comment `3653191687` identified that `normalizedSpecWarningToIssue` hard-codes `/resources` although every warning carries a structured `location`                                                              | Preserve a valid normalized-contract JSON Pointer for request, inline-response, and canonical-response warnings; rerun Gen and CLI tests    | 3     | DONE   |
| The command plugin rejects `version` even though the generated runtime has no conflicting version command             | Review comment `3653191692` identified `version` in `RESERVED_COMMAND_NAMES`; the runtime only recognizes `--help` and exposes no `version` subcommand or `--version` switch                                           | Prove an operation named `version` validates and generates while the actual reserved-name contract remains deterministic                    | 3     | DONE   |
