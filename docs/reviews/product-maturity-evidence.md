...
- **G3 — Stage 3:** every command in the full repository gate passed under Node 24.16.0 and pnpm
  ...
  and publish dry run passed; the final `git status --short` was empty. Stage 3 pull-request checks
  remain pending.
...
completion still depends on an open, green, unmerged Stage 3 pull request at its recorded head.
