...
that directory (for example `fixtures.ts`). Do not copy setup between test files. Pass deliberately
invalid input through a suite's named unchecked-input helper instead of a type assertion. Vitest
gates that select tests by path name suite directories or unsplit test files, which
`pnpm verify:test-gates` enforces, so a later split stays covered. Run the CLI locally with
